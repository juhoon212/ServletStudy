...
public class SessionServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// => 매개 변수에 값을 구분하기 위한 속성명과 연결 지속성을 제공하기 위한 속성값을 전달
		// 전달받은 속성명과 같은 이름의 속성값이 존재할 경우 덮어씌우기 - 속성값 변경
		// 동일한 세션을 바인딩한 모든 웹프로그램에게 속성명으로 속성값을 제공받아 사용 가능
		
		session.setAttribute("now", new Date());
		// HttpSession.getAttribute(String attributeName, Object attributeValue)
		// 반환하는 메소드 - 매개변수에 속성값을 구분하기 위한 속성명 전달
		// => HttpSession 객체에 저장된 속성값을 Object 객체로 반환하므로 반드시 명시적 객체 형변환 사용
...
//		
//		//HttpSession.invalidate() => 바인딩된 세션을 언바인딩 처리하는 메소드
//		session.invalidate();
//		
		out.println("</body>");
		out.println("</html>");
